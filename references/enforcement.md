...
**L0–L2 are baseline for every harness.** L3 adds three specific hooks that target unrecoverable failure modes. L4 is documentation of intent — useful for human readers, but real enforcement happens at L3.
...
Opening Claude Code in the project lands on the coordinator. The user can't forget to use the harness because there's nothing else to use.

---

## Layer 2 — Stop hook QA gate (with diff-guard prefix)

Fires after every Claude response. Must be **agent-type** (not command-only) so it can reason about "does this match the plan?" not just "does this parse?"
...
When gstack is installed, its safety skills add real protection at this layer: `/careful` warns before destructive commands, `/freeze` locks edits to a directory, `/guard` combines both. These activate per-session when working in high-risk areas.
